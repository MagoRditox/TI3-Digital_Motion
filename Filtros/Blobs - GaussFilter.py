...
for k in range (0,1212,1):
    ...
    try:
        ...
        #los parametros que nos pide este filtro es primeramente
        #el primer parametro es la  imagen de entrada
        #el segundo parametro es el tamaño del kernel gaussiano (ancho y alto), el ancho y alto pueden tener valores diferentes y deben ser positivos e impares
        #y el tercer valor especifica los límites de la imagen mientras se aplica el kernel en los bordes de la imagen.
        img_small = cv2.GaussianBlur(gray_img,(7,7),cv2.BORDER_DEFAULT)

        img_big = cv2.GaussianBlur(gray_img,(7,7),cv2.BORDER_DEFAULT)
        #El filtrado gaussiano es el proceso de promedio ponderado de toda la imagen.
        #El valor de cada píxel se obtiene mediante el promedio ponderado de sí mismo y otros valores de píxeles en la vecindad.
        # Circulos Pequeños
