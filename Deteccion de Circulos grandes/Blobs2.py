...
inicio = time.time()
img_read = cv2.imread('Seq/img001007.jpg')  # Imagen de entrada
...
if big_circles is not None:
    ...
    for i in big_circles[0, :]:
        ...
        if b in range(70,150) and g in range(70,150) and r in range(70,150) or b == 0 and g == 255 and r == 255:
            continue
        elif b > r:
            # Dibujar el circulo
            # parametros(imagen,centro,radio,color,grosor del contorno a dibujar)
...
fin = time.time()
promedio = fin-inicio
print ("El tiempo de ejecucion es :" +str(float(promedio))+ " segundos")
